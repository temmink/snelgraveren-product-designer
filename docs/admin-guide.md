...
| Build System | Vite (single config, dual entry points) |
| Canvas Engine | Fabric.js 6.x |
| State Management | Zustand (both admin and frontend) |
| PHP Autoloading | PSR-4 (`ProductForge\` namespace) |
| Database | Custom tables with InnoDB, foreign keys, prepared statements |
| Export | TCPDF (PDF), Intervention Image (PNG), Pure PHP (SVG) |
...
├── admin/js/template-builder/    # Admin React app (Vite)
├── frontend/js/designer/        # Frontend React app (Vite)
...
| **Page size** | Canvas dimensions (width × height in points) |
| **Rendered elements** | Background color/image, text (with font, size, weight, style, color), images, paths, groups, rects, circles |
| **Custom fonts** | Uploaded custom fonts are embedded in the PDF via `addTTFfont()` |
| **Background images** | Resolved to local filesystem path for embedding |

### PNG Export

| Feature | Detail |
|---------|--------|
| **Library** | Intervention Image (GD/Imagick) |
| **DPI** | 300 (default) |
| **Multi-view** | Separate PNG file per view (suffixed `-view-N`) |
| **Rendered elements** | Background, text, images, basic shapes |
...
| **Multi-view** | Separate SVG file per view |
| **Rendered elements** | Background color/image (base64 embedded), text, images, paths, groups (recursive), rects, circles |
| **Custom fonts** | Embedded as base64 `@font-face` within `<defs>` |
...
ProductForge creates 8 tables (all using InnoDB engine):
...
| file_path | VARCHAR(2048) | Absolute server path |
...
| POST | `/designs/{hash}/views` | Owner | Upsert view (canvas JSON + thumbnail) |
...
| DELETE | `/exports/{id}` | Admin | Delete export |
...
### Exports failing
- **PDF**: Ensure TCPDF library is available (included via Composer).
- **PNG**: Ensure GD or Imagick PHP extension is installed.
- **SVG**: No external dependencies — if failing, check PHP error log for the specific error.
...
- Check the `pf_db_version` option value — it should match the latest migration number (500).
