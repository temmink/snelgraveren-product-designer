...
- **No `stopPropagation` on `#pf-designer-root`:** The designer renders inside a WooCommerce product tab via `[productdesigner]` shortcode. Never add blanket `stopPropagation` on the designer root — it blocks Fabric.js's `document`-level `pointerup` listener, causing dragged objects to stick to the cursor.
...
- Shortcode: `[productdesigner]` renders the designer inline on product pages (auto-detects product context). Alias `[sgpd_designer]` maps to the same callback. Both are detected by `has_shortcode_in_content()` for the auto-render dedup. (The legacy `[productforge]` tag was removed in v1.7.6 — existing live content must switch to `[productdesigner]`.)
