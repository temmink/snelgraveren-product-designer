...
- **wordpress.org release (approved 2026-08, slug `snelgraveren-product-designer`):** SVN `https://plugins.svn.wordpress.org/snelgraveren-product-designer`, listing `https://wordpress.org/plugins/snelgraveren-product-designer/`. Current version 1.7.8 (starter-boundary fix); nothing committed to SVN yet as of 2026-08-20. Run `bash bin/wporg-release.sh` (`--dry-run` to stage only) on the Mac: it runs `package.sh` + `free-build.sh`, sanity-checks the FREE zip (no premium paths, no gettext files, gatekeeper stripped, `is_premium => false`), syncs it into `trunk/`, copies `wporg-assets/` (icon/banner/`screenshot-N.png`) into SVN `assets/`, copies `trunk` → `tags/<version>/` and commits. Checkout lives in `.svn-wporg/` (git-ignored). wp.org gets the **free** build only; the premium zip still goes to Freemius → Deployment. Tags are immutable — bump the version for any fix, never re-tag. SVN username = wordpress.org username (`snelgraveren`, case-sensitive), password = the separate SVN password from the wp.org profile.
