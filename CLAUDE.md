...
- **Config read retry loop (`App.jsx`):** On first render the helper may return `{}` — seen in the wild on Safari with LiteSpeed's Delay JS, and in private-mode browsers. `App.jsx` therefore stores config in `useState`, and an effect polls `getDesignerConfig()` every 50 ms (up to 40 attempts / ~2 s) calling `setConfig` once a real `template_id` appears. The template-loading effect depends on `config.template_id`, not `[]`, so it re-runs after the poll succeeds. Never revert to a single synchronous module-level read — it breaks on real Safari even when the DOM inspector shows the attribute.
...
- **JS cache busting (hash in filename, not `?ver=`):** `Frontend::enqueue_assets()` copies `dist/frontend-designer.js` to `dist/frontend-designer.<md5>.js` on the first pageview after a build and enqueues the hashed URL. Old hashed copies are deleted during the copy step. **Why not `?ver=<hash>` alone:** LiteSpeed's Delay JS feature strips query strings from `<script>` tags it marks with `data-deferred="1"`, so Safari (which caches JS with `max-age=31557600` ≈ 1 year) keeps serving stale bundles after deploys. Changing the path itself is the only reliable invalidator. If the plugin dir is read-only, the copy fails silently and we fall back to the un-hashed URL.
