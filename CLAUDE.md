...
- Shortcode: `[productdesigner]` renders the designer inline on product pages (auto-detects product context). Aliases `[productforge]` (old plugin name, kept for existing live content) and `[sgpd_designer]` map to the same callback. All three are detected by `has_shortcode_in_content()` for the auto-render dedup.
