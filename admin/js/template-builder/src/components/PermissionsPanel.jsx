...
export default function PermissionsPanel() {
  ...
  return (
    <div className="pf-permissions">
      {ELEMENT_TYPES.map(({ key, label, extras }) => {
        const perm = getTypePerm(key);
        const boolKeys = ['resize', 'rotate', 'delete', ...extras, 'snap_to_grid'];
        return (
          <fieldset key={key} className="pf-permissions__group">
            <legend>{label}</legend>
            {boolKeys.map((bk) => (
              <label key={bk} className="pf-permissions__check">
                <input
                  type="checkbox"
            ...
            <div className="pf-permissions__scales">
              <label>
                ...
                <input
                  ...
                  className="pf-permissions__number"
                />
              </label>
              <label>
